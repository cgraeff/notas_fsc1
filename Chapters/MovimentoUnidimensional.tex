...
\comment{Fig inline reta e bloco, mais nada}

\noindent{}Claramente, tal descrição é insuficiente. Para determinar a posição do corpo, precisamos de um \emph{ponto de referência}. A partir desse ponto, podemos então determinar a posição medindo a distância, que denominamos como \emph{módulo}, entre ele e o corpo:

\comment{Fig inline reta e bloco e origem, mais medida entre origem e bloco}

Tal descrição ainda é insuficiente, pois podemos ter outro objeto que pode estar à mesma distância da origem:

\comment{mesma figura de antes, mas com mais um bloco à mesma distância, porém do outro lado}
...
\comment{Fig com origem como zero, com números negativos e positivos, dois blocos numerados, um na região negativa e outro na positiva, seta para a direita}
...
Se ocorre movimento, podemos dizer que a cada instante de tempo $t$, temos um valor de posição $x$ diferente. Se nos lembrarmos do conceito de funções, temos que dados dois grupos de números, uma função é a operação matemática que liga elementos do primeiro grupo a elementos do segundo\footnote{Lembre-se de que dois elementos do grupo $x$ podem levar a um mesmo elemento do grupo $y$, porém um elemento de $x$ não pode levar a dois elementos de $y$.}:

\comment{inline da classica figura com dois conjuntos com vários pontinhos e flechinhas que ligam os pontinhos do primeiro grupo a elementos do segundo grupo. Nas flechinhas, label $y = f(x)$, nos grupos, label $x$ e $y$. ligar dois pontos de $x$ a um mesmo ponto de $y$}
...
Gráficos, em analogia ao caso da posição (parado, vel. cte., etc).
...
\comment{Fazer a interpretação gráfica da aceleração média nos moldes da velocidade média.}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Aceleração instantânea}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\comment{Gráfico de $v(t)$ variável com $t_1$ fixo e várias retas entre $(t_1, v_1)$,  $(t_2, v_2)$, $(t_3, v_3)$, etc., para mostrar que a reta tente a uma tangente ao ponto $(t_1, v_1)$ no limite de $\Delta t \to 0$. Fazer nos moldes da velocidade instantânea, mas ser mais direto.}
