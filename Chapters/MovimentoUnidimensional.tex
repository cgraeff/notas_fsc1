...
\begin{tikzpicture}[>=Stealth, extended line/.style={shorten >=-#1,shorten <=-#1},
 extended line/.default=4mm]]
    ...
    \path[name path=froma](a)--+(0,4);
    \path[name path=fromb](b)--+(0,4);
    \draw[dotted, thin,name intersections={of=froma and plot}](a)--(intersection-1) coordinate (plot-a-intersection)--(0,0|-intersection-1)node[left]{$v_1$};
    \draw[dotted, thin, name intersections={of=fromb and plot}](b)--(intersection-1) coordinate (plot-b-intersection)--(0,0|-intersection-1)node[left]{$v_2$};
