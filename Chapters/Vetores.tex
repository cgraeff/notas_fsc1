...
\begin{tikzpicture}[>=Stealth]
    ...
    \draw[->] (ai) +(-1.5,0) -- +(1.25,0) node[below left]{$x$};
    \draw[->] (ai) -- +(0,2) node[below right]{$y$};
    \draw[->, very thick] (ai) -- +(0.5,0) node[below] {$\versi$};
    \draw[->, very thick] (ai) -- +(0, 0.5) node[right]{$\versj$};
        
    \draw[->, thick] (ai) -- +(-1.25,0) node[below right]{$\vec{a}_x$};
    \draw[->, thick] (ai) -- +(0,1.5) node[below right]{$\vec{a}_y$};
