...
\draw[->, dashed] (bob) -- +(0,1.5,0) node[below right]{$y$};
...
\pic [draw, "$\theta$", angle eccentricity=1.5] {angle = center--origin--bob};
\end{tikzpicture}
\caption{Diagrama de forças do pêndulo cônico. Note que a força centrípeta pode ser identificada nesse caso com a componente da tensão no eixo $x$ apresentado na figura.}
\end{marginfigure}
