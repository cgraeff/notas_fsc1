...
Quando mais que uma força atua sobre um corpo, devemos determinar a \emph{força resultante} que atua sobre ele para que possamos determinar qual será a aceleração a qual eele estará submetido. Isso pode ser feito de maneira relativamente simples, bastanto utilizar as propriedades vetoriais, já que as forças são vetores. Assim,
\begin{equation}
...
Um artifício fundamental para a interpretação e solução de problemas de dinâmica é o \emph{diagrama de corpo livre}, ou \emph{diagrama de forças}. Tal diagrama consiste em um ponto que representa um corpo, sendo que todas as forças que atuam sobre tal corpo são representadas como atuando sobre o ponto (veja a parte à direita
na Figura~\ref{Fig:DiagramaDeForcasEEsboco}). Além disso, não devemos incluir acelerações, velocidades, ou quaisquer outros vetores no diagrama de forças.

Podemos, ao invés de utilizar um diagrama de forças como descrito acima, fazer um esboço da situação (veja a parte à esquerda
na Figura~\ref{Fig:DiagramaDeForcasEEsboco}). Isso em geral é mais interessante, pois ele reúne as principais características de um diagrama de forças e ao mesmo tempo permite uma visualização do problema. Esse artifício exige algumas adaptações, como representar forças em posições diferentes das ideais para que elas possam ser representadas confortavelmente no esboço.
...
Na Figura~\label{Fig:ExemploEquilibrio} temos um corpo sujeito a um conjunto de forças e em equilíbrio. Podemos determinar a relação entre as forças através da Segunda Lei de Newton. Para isso, vamos adotar um sistema de referência e determinar o ângulo entre as forças e os eixos. Veja a Figura~\ref{Fig:ExemploEquilibrioRef}. Aplicando a Segunda Lei de Newton a cada eixo, temos
\begin{description}
    \item[Eixo $x$:]
        \begin{align}
            F_R^x &= m a_x \\
...
\begin{tikzpicture}[>=Stealth, rotate=-35,
    ...
    \pic [draw, "$\theta$", angle eccentricity=1.5] {angle = O--A--B};
    \pic [draw, "$\beta$", angle eccentricity=1.5] {angle = B--O--P};
...
\begin{description}
    \item[Eixo $x$:] No eixo paralelo ao plano temos
        \begin{align}
            ...
            mg \sen\theta &= ma_x,
        \end{align}
        onde utilizamos as funções trigonométricas para determinar a componente do vetor peso na direção do eixo $x$. Finalmente, ao dividirmos ambos os lados da equação pela massa, obtemos
        \begin{equation}
            a_x = g \sen\theta.
        \end{equation}
    \item[Eixo $y$:] No eixo perpendicular ao plano temos
        \begin{align}
            ...
            N &= mg \cos\theta,
        \end{align}
        onde utilizamos o fato de que não há aceleração no eixo perpendicular ao plano para escrever o termo à direita da igualdade no segundo passo.
\end{description}

Verificamos, portanto, que a aceleração em um plano inclinado varia conforme alteramos a inclinação deste.
...
Um tipo de problema relativamente comum envolve um objeto sustendado por cordas. As cordas podem estar atadas umas às outras, o que nos leva uma uma situação como a da Figura~\ref{Fig:BlocoSustentadoPorCordas}. Nesse tipo de situação, podemos imaginar que as cordas estão ligadas a um anel, por exemplo. Precisamos, portanto, aplicar a Segunda Lei de Newton ao anel, para que possamos relacionar as forças exercidas pelas cordas.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{Atrito e equilíbrio}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Na Figura~\ref{Fig:BlocoPlanoInclinadoAtritoEquil} temos uma situação baste comum: um sistema em equilíbrio devido à força de atrito. Neste problema, apesar de o atrito ser estático, ele não é --~ao menos a princípio~ estático máximo. Assim, só podemos extrair informações dele através da Segunda Lei de Newton:
\begin{description}
    \item[Eixo $x$:]
        \begin{align}
            F_R^x &= m a_x \\
...
\begin{tikzpicture}[>=Stealth, rotate=-35,
    ...
    \draw[->, thick] (1.5,0) -- +(-0.5735,0) node[above]{$\vec{f}_{at}$};
...
\end{tikzpicture}
\caption{Bloco sobre plano inclinado, em equilíbrio devido à força de atrito.\label{Fig:BlocoPlanoInclinadoAtritoEquil}}
\end{marginfigure}

\noindent{}Usamos nas equações acima o fato de que a aceleração é zero. Decompondo o peso,  sabendo que o ângulo entre o peso e o eixo $y$ é $\theta$, concluímos que
\begin{equation}
    f_{at} = P\sen\theta.
\end{equation}
...
Uma escolha razoável para o eixo $x$ é adotá-lo como paralelo ao plano inclinado. Dessa forma o ângulo entre a força peso e o eixo $y$ será o mesmo que aquele entre o plano inclinado e a horizontal. As demais forças estarão em um eixo somente.
...
\begin{tikzpicture}[>=Stealth, rotate=-35,
    ...
    \pic[draw, "$\theta$", angle eccentricity=1.5] {angle = X--center--P};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{Atrito e aceleração}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

A força de atrito pode ser a \emph{causa} da aceleração de um sistema. Um exemplo simples disso é um bloco que é impulsionado e passa a se mover sujeito a uma força de atrito cinético. Outro exemplo é o de um bloco sobre uma esteira, sendo que esta acelera o bloco através do atrito. Nesse casos temos, através da Segunda Lei de Newton:
\begin{description}
    \item[Eixo $x$:]
        \begin{align}
            F_R^x = m a_x \\
            f_{at}^e = m a_x.\label{Eq:BlocoAceleradoPeloAtritoX}
        \end{align}
    \item[Eixo $y$:]
        \begin{align}
            F_R^y &= m a_y \\
...
\begin{tikzpicture}[>=Stealth,
    ...
    \draw[fill] (0, 0.5) circle (1pt);
    \draw[->, thick] (0, 0.5) -- +(0,-1) node[right]{$\vec{P}$};
    \draw[->] (0,1) -- +(0,1) node[below right]{$\vec{N}$};
    
    \draw[->, thick] (0.5,0) -- +(1,0) node[above]{$\vec{f}_{at}^e$};
    \draw[->, thick, gray] (-0.5,0) -- +(-1,0) node[above]{$\vec{f}_{at}^c$};
    
    \draw[->](-0.6, 2.8) -- node[above]{$\Delta\vec{r}$} +(1.2,0);
    \draw[|->](0,2.5) -- node[below]{$\vec{a}$}+(0.4,0);
    \draw[->,gray](0,2.5) -- node[below]{$\vec{a}$}+(-0.4,0);
\end{tikzpicture}
\caption{Exemplos de acelerações causadas pelo atrito: no caso de um bloco que se desloca para a direita, sujeito a uma força de atrito estático que o acelera --~como, por exemplo no caso de o bloco estar sobre uma esteira~--, temos uma aceleração \emph{devido ao atrito}. Outra possibilidade é a de o atrito ser cinético, e nesse caso ele apontaria na direção oposta ao movimento (em cinza na figura).}
\end{marginfigure}

\noindent{}Utilizamos acima o fato de que a aceleração é zero no eixo $y$. Além disso, tratamos o caso em que a força de atrito é $f_{at}^e$ na figura. O resultado seria idêntico, somente alterando o sentido da aceleração, se tivéssemos escolhido $f_{at}^c$.

Note que, no caso em que temos uma aceleração devido à força de atrito estático, não podemos afirmar ser uma força de atrito estático máxima. Logo, o módulo da força deve ser obtido através da Equação~\eqref{Eq:BlocoAceleradoPeloAtritoX}. No entanto, se estivermos tratando do atrito cinético, podemos afirmar que $f_{at}^c = \mu_c N$.
...
\end{tikzpicture}
\caption{Forças em um movimento circular vertical executado por um corpo preso a um fio.}
\end{marginfigure}
...
\end{align}
%
ou seja, a velocidade máxima com que podemos fazer a trajetória circular sem perder contato\footnote{Isto é, estamos na iminência de perder contato.} é
\begin{equation}
    v = \sqrt{rg}.
\end{equation}
...
\end{equation}
%
Substituindo essa equação a primeira equação do sistema, resulta em
\begin{equation}
...
\begin{tikzpicture}[>=Stealth, scale = 1.2,
    ...
    \draw[pattern = north west lines, pattern color = gray, draw = gray] (1,0) rectangle (2,1);
    \draw[pattern = north east lines, pattern color = gray, draw = gray] (2,0) rectangle +(1.5,1.5);
    \draw[pattern = north west lines, pattern color = gray, draw = gray] (2.25,1.5) rectangle +(1,1);
    ...
    \draw[thick, ->] (2.45, 1.5) -- +(0,-0.5) node[right]{$\vec{N}'_3$};
    
    \draw[thick, ->] (3.25, 1.6) -- +(0.5,0) node[above]{$\vec{f}_{at}$};
    \draw[thick, ->] (2,1.4) -- node[above]{$\vec{f}'_{at}$} +(-0.5,0);
    
    \draw[thick, ->] (2,0.85) -- node[below]{$\vec{F}_{12}$} +(0.65,0);
    \draw[thick, ->] (2,0.85) -- node[below]{$\vec{F}'_{12}$} +(-0.65,0);
...
\noindent{}Aplicando o teorema de Pitágoras, temos que
\begin{equation}
    F_3^2 = F_1^2 + F_2^2.
\end{equation}
