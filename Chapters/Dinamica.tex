...
\begin{question}[type={exam}]\label{Q:DistencaoMaxMassaMolaParaSegundoBlocoNaoDeslisar}
A figura abaixo mostra um bloco que está apoiado sobre uma mesa sem atrito e que está ligado a uma mola. Sobre este bloco, repousa um segundo bloco. Na interface entre os blocos, existe atrito. Qual é a maxima distensão $x_m$ que a mola pode ter para a qual os blocos aceleram juntos ao liberarmos o sistema para se mover? Considere que $k = \np[N/m]{120.0}$, $m_1 = \np[kg]{3,0}$, $m_2 = \np[kg]{1.0}$, e $\mu_e = \np{0.65}$.
...
\end{tikzpicture}
\caption{Questão~\ref{Q:DistencaoMaxMassaMolaParaSegundoBlocoNaoDeslisar}.}
\end{marginfigure}
\end{question}

\begin{question}[type={exam}]\label{Q:BrinquedoParqueTuboAtrito}
A figura abaixo mostra um brinquedo de um parque de diversões. Os ocupantes sobem em uma plataforma circular que é então levantada de forma a fechar a parte inferior de um cilindro. Após isso, todos se dispõe em pé, com as costas tocando a parede cilíndrica, e o sistema começa a girar. A ``diversão'' consiste em descer a plataforma enquanto o sistema gira, sendo que os passageiros permanecem presos à parte cilíndrida devido à força de atrito. Se o coeficiente de atrito entre a parede e os ocupantes é de \np{0,65}, e o raio do cilindro é \np[m]{3,0}, qual é o valor mínimo do módulo da velocidade dos ocupantes para eles que não caiam quando a plataforma se abrir?
