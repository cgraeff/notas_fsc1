...
Toda situação onde não existe um perfeito equilíbrio das forças que atuam em um corpo implica na existência de uma aceleração. Se considerarmos a Figura~\ref{}, verificamos a existência de quatro forças atuando sobre um corpo. Assumimos que não há equilíbrio em tal sistema, por isso devemos levar em conta a aceleração devida à força resultante. Para que possamos determinar o vetor aceleração mais facilmente, escolhemos o sistema de coordenadas mostrado na Figura~\ref{}.
...
\begin{tikzpicture}[>=Stealth]
    ...
    \draw[->] (0.5,1) -- +(0,1) node[left]{$\vec{F}_1$};
    \draw[->] (1,0.5) -- +(1,0) node[below]{$\vec{F}_2$};
    \draw[->] (0,0) -- +(-0.8,-1.3) node[left]{$\vec{F}_3$};
    \draw[->] (1,1) -- +(0.5,0.4) node[above]{$\vec{F}_4$};
\end{tikzpicture}
\caption{.}
\end{marginfigure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{Discussão: Sistema fora do equilíbrio de forças, com movimento limitado}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Em diversas situações, um sistema físico está sujeito a uma série de \emph{vínculos} que limitam o movimento a uma só direção. Dessa forma, se há aceleração, a direção já é pré determinada

\textbf{Enfatizar a escolha do sistema de coordenadas}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\paragraph{Direção aparente do peso}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Quando um avião faz uma curva, não sentimos uma alteração da direção do peso em relação ao assoalho da aeronave. Explicar o por quê. Não tenho certeza absoluta, mas acredito que está ligado ao fato de que a força de sustentação é na mesma direção que a normal para os passageiros. Para que sentíssemos um peso não perpendicular ao chão, temos que ter alguma aceleração lateral em relação à aeronave, coisa que a normal ou mesmo a força de sustentação não são capazes de prover. Dá pra interpretar que o campo gravitacional efetivo tem uma componente radial para fora igual à centrípeta e a própria aceleração gravitacional, aí a resultante é perpendicular ao assoalho
