...
\begin{tikzpicture}[>=Stealth]
    ...
    \draw[draw = gray, pattern = north west lines, pattern color = gray] (0,3) circle (2mm);
    \draw[gray, fill] (0,3) circle (1pt);
    \draw[->, thick, gray] (0,3) -- +(0,-1) node[right]{$\vec{P}$};
    
    \draw[->] (-1, -1) -- (-1, 4) node[below left]{$y$};
    \draw[|-|] (-1,0) node[left]{$y_i = 0$} -- (-1,3) node[left]{$y_f = h$};
...
\begin{quote}
\emph{se o trabalho realizado por uma força no deslocamento entre dois pontos é depende do caminho, não podemos escrever um potencial para tal força.}
\end{quote}
...
% TODO Reescrever isso abaixo
Diversos sistemas físicos podem ser interpretados de uma maneira mais simples utilizando potenciais. É muito comum que tenhamos informações diretas somente sobre o potencial, pois isso é mais fácil de medir (pense no potencial elétrico). Isso se deve ao fato de que a energia é um escalar, não possuindo direção e sentido. Nesses sistemas, é comum que se faça uma análise do gráfico da energia potencial. Veremos a seguir um simples gráfico do potencial em função da posição encerra várias informações interessantes.
...
\begin{tikzpicture}[>=Stealth, scale = 1.3, rotate around y = -15, rotate around z = -15, extended line/.style={shorten >=#1,shorten <=#1},
 extended line/.default=2mm]]
    ...
    \draw[dotted, ->] (0,0,-2) -- (0,0,2) node[below]{$x$};
    ...
    \draw[densely dotted, extended line] (H1) -- (H2);
    \draw[densely dotted, extended line] (H2) -- (H3);
    \draw[densely dotted, extended line] (H3) -- (H1);
    
    \draw[pattern = north east lines] (0,0,0.5) coordinate (N) circle (1.5mm);
    \draw[pattern = north east lines, pattern color = gray, draw = gray] (0,0,-0.5) circle (1.5mm);
    \draw[densely dotted, extended line] (N) -- (H1);
    \draw[densely dotted, extended line] (N) -- (H2);
    \draw[densely dotted, extended line] (N) -- (H3);
...
Devido às forças entre as partículas, pode ocorrer a transferência de energia cinética ou potencial entre elas, porém a energia mecânica do sistema como um todo se mantém constante. Esse tipo de sistema é denominado um \emph{sistema fechado}.

Mesmo em casos onde o sistema não é fechado, se soubermos qual é o trabalho total realizado sobre o sistema, podemos saber qual é a energia cinética total, pois

% Generalizar a definição de energia mecãnica
...
Se voltarmos ao problema do oscilador massa-mola (Figura~\ref{Fig:PotElasticoComMassaMola}), mas desta vez considerarmos a possibilidade da existência de uma força de atrito, teremos que o sistema oscilará durante um tempo, mas eventualmente parará. Se o sistema partir de um ponto $x_A$ à esquerda da origem, por exemplo, podemos verificar que após um deslocamento até $x_B$, o atrito realizará um trabalho sobre o bloco dado por
\begin{equation}
  W_{\fat} = - \fat \Delta x.
\end{equation}
%
...
discutir o problema da origem da energia cinética de um carro (nenhum trabalho atravessa a fronteira, então a energia é interna, eu acho). Antes, discutir o problema de dois blocos ligados por uma mola e que saltam, como uma caneta com botão.
