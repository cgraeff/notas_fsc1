...
\begin{tikzpicture}[>=Stealth]
    ...
    \draw[pattern = north west lines, dotted] (3.5,0) coordinate (c2) circle (2mm);
...
\textbf{Precisamos esclarecer aqui que se existem forças que não realizam trabalho, não precisamos levá-las em conta. No caso de forças conservativas, isso se deve ao fato de que a variação do potencial é nula. No caso de forças não conservativas, tb não é preciso levar em conta, mas vamos entender o por quê só um pouco adiante, na seção de Trab. de Forças não conservativas/Energia interna. Isso poderia até ficar como um footnote.}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{Discussão: Pêndulo simples}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\textbf{Discutir o pêndulo antes, mas de maneira mais simples do que está feito para o oscilador harmônico abaixo. Mesmo assim, talvez seja preciso mexer no que está abaixo. Outra ideia é fazer algo bem simples, parecido com o exercício 32b da lista, mas mais simples.}
...
\begin{tikzpicture}[>=Stealth,
    ...
    \draw[->] (0,-3.5) -- (4.8,-3.5) node[below left]{$x$};
    \draw[|-|] (2.5,-3.5) node[below]{$0$} -- (4,-3.5) node[below]{$A$};
    \draw[|-|] (2.5,-3.5) -- (1,-3.5) node[below]{\llap{$-$}$A$};
...
\textbf{Discutir genericamente a questão de equilíbrio: analisar o que acontece com a força em cada situação. Tirar  a molécula de amônia do meio da discussão e a colocar em um paragraph.}
...
\begin{itemize}
    ...
    \item \textbf{Falar dos pontos de sela.}
\end{itemize}

\textbf{Interpretar essa questão do ponto de máximo ou mínimo para o caso da força: se num ponto temos que a derivada é zero, se a derivada segunda é positiva, isso quer dizer que existe uma tendência a: no ponto imediatamente à esquerda do mínimo, de a inclinação ficar ``menos negativa''; já no ponto imediatamente à direita, ficar ``mais positiva''. Fazer gráfico, senão ninguém vai entender. (É como se a derivada ser positiva no ponto causasse uma rotação da reta tangente no sentido anti-horário; no caso de ser negativa, há uma tendência a rotacionar a reta no sentido horário.)}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{Exemplo: Molécula de amônia}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\textbf{Adaptar essa seção para que ela seja independente. (costumava ser parte do texto da seção anterior.}

Essa classificação é simples de se entender através da Figura~\ref{Fig:MoleculaAmoniaEPotencial}, onde mostramos uma molécula de amônia. O átomo de nitrogênio pode ocupar três posições de equilíbrio: uma de cada lado do plano formado pelos três átomos de hidrogênio e a posição central, exatamente no meio de tal plano. De acordo com o potencial mostrado na figura, se tomarmos qualquer uma das posições fora do plano, temos que um deslocamento ao longo do eixo $x$ em qualquer direção dá origem a uma força que tende a \emph{restaurar} o nitrogênio à posição inicial: um deslocamento no sentido positivo de $x$ dá origem a uma força no sentido negativo, enquanto um deslocamento no sentido negativo de $x$ dá origem a uma força no sentido positivo.
