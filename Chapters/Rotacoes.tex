...
As medidas eu graus ou grados são obtidas em comparação com um padrão pré-definido, porém menor do que uma revolução. De certa forma, continuamos adotando frações de um círculo, pois usamos múltiplos de uma pequena fração que corresponde a \degree{1}: $\nicefrac{1}{2}$ revolução equivale a $\np[\tcdegree]{360} / 2 = \np[\tcdegree]{180}$, $\nicefrac{1}{3}$ de revolução equivale a $\np[\tcdegree]{360} / 3 = \np[\tcdegree]{120}$, etc. Para a medição de um ângulo na prática, necessitamos usar um transferidor, ou seja, realizamos uma comparação com uma escala.
...
\begin{tikzpicture}[>=Stealth,
    ...
    \path[name path=up_from_origin_1] (0,0)--+(0,1);
    \path[name path=down_from_origin_1] (0,0)--+(0,-1);
    \path[name path=up_from_origin_2] (3,0)--+(0,1.5);
    \path[name path=down_from_origin_2] (3,0)--+(0,-1.5);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{Discussão: Torque devido ao peso de um corpo rígido}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\textbf{Provar que o torque devido ao peso de um corpo rígido pode ser considerado como o torque de uma partícula que ocupa a posição do centro de massa.}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{Discussão: Balanças de braços}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{tikzpicture}[interface/.style={
    ...
    \draw[|<->] (0.6, 2) -- node[above]{$\ell$} (1.5,2);
...
Precisamos ainda demostrar no contexto de rotações a validade da expressão
\begin{equation}
    \Delta K = W.
\end{equation}
%
...
%%% Teto
    ...
    \draw[interface] (-3,2.25) -- (-2.45,-2.5);
...
Aplicando a Segunda Lei de Newton ao centro de massa obtemos
\begin{description}
\item[Eixo $y$:]
\begin{align}
